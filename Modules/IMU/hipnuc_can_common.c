...
int hipnuc_can_to_json(const can_sensor_data_t *data, int msg_type, can_json_output_t *output) {
    ...
    output->length = 0;
    output->buffer[0] = '\0';
    json_append(output, "{\"node_id\":%d,\"hw_ts_us\":%llu,\"data\":{", data->node_id,
                (unsigned long long) data->hw_ts_us);
    switch (msg_type) {
        case CAN_MSG_ACCEL:
        ...
        case CAN_MSG_MAG:
            ...
            break;
        case CAN_MSG_QUAT:
            json_append(output, "\"quat_w\":%.4f,\"quat_x\":%.4f,\"quat_y\":%.4f,\"quat_z\":%.4f", data->quat_w,
                        data->quat_x, data->quat_y, data->quat_z);
            break;
        case CAN_MSG_EULER:
        ...
        case CAN_MSG_TIME:
            json_append(
                output,
                "\"utc_year\":%d,\"utc_month\":%d,\"utc_day\":%d,\"hours\":%d,\"minutes\":%d,\"seconds\":%d,\"milliseconds\":%d,\"timestamp_ms\":%u",
                data->utc_year, data->utc_month, data->utc_day, data->hours, data->minutes, data->seconds,
                data->milliseconds, data->timestamp_ms);
            break;
        case CAN_MSG_GNSS_POS:
            json_append(
                output, "\"ins_lon\":%.8f,\"ins_lat\":%.8f,\"ins_msl\":%.3f,\"undulation\":%.2f,\"diff_age_s\":%.2f",
                data->ins_lon, data->ins_lat, data->ins_msl, data->undulation, data->diff_age_s);
            break;
        case CAN_MSG_GNSS_VEL:
            json_append(output, "\"ins_vel_e\":%.3f,\"ins_vel_n\":%.3f,\"ins_vel_u\":%.3f,\"ins_speed\":%.3f",
                        data->ins_vel_e, data->ins_vel_n, data->ins_vel_u, data->ins_speed);
            break;
        case CAN_MSG_GNSS_STATUS:
            json_append(output, "\"solq_pos\":%d,\"solq_heading\":%d,\"nv_pos\":%d,\"nv_heading\":%d,\"ins_status\":%d",
                        data->solq_pos, data->solq_heading, data->nv_pos, data->nv_heading, data->ins_status);
            break;
        default:
...
static void hipnuc_j1939_build_cmd(uint8_t da, uint8_t sa, uint16_t addr, hipnuc_j1939_cmd_t cmd, uint32_t val,
...
int hipnuc_j1939_parse_cmd(const hipnuc_can_frame_t *frame, uint16_t *addr, hipnuc_j1939_cmd_t *cmd, uint8_t *status,
                           uint32_t *val) {
